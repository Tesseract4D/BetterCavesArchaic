...
import cn.tesseract.mycelium.shadow.org.joml.Vector3i;
...
{
    DOWN(0, 1, -1, "down", EnumFacing.AxisDirection.NEGATIVE, EnumFacing.Axis.Y, new Vector3i(0, -1, 0)),
    UP(1, 0, -1, "up", EnumFacing.AxisDirection.POSITIVE, EnumFacing.Axis.Y, new Vector3i(0, 1, 0)),
    NORTH(2, 3, 2, "north", EnumFacing.AxisDirection.NEGATIVE, EnumFacing.Axis.Z, new Vector3i(0, 0, -1)),
    SOUTH(3, 2, 0, "south", EnumFacing.AxisDirection.POSITIVE, EnumFacing.Axis.Z, new Vector3i(0, 0, 1)),
    WEST(4, 5, 1, "west", EnumFacing.AxisDirection.NEGATIVE, EnumFacing.Axis.X, new Vector3i(-1, 0, 0)),
    EAST(5, 4, 3, "east", EnumFacing.AxisDirection.POSITIVE, EnumFacing.Axis.X, new Vector3i(1, 0, 0));
    ...
    private EnumFacing(int indexIn, int oppositeIn, int horizontalIndexIn, String nameIn, EnumFacing.AxisDirection axisDirectionIn, EnumFacing.Axis axisIn, Vector3i directionVecIn)
    {
        this.index = indexIn;
    ...
    {
        return byHorizontalIndex(MathHelper.floor_double(angle / 90.0D + 0.5D) & 3);
    }
    ...
    {
        if (Math.abs(placer.posX - (double)((float)pos.x + 0.5F)) < 2.0D && Math.abs(placer.posZ - (double)((float)pos.z + 0.5F)) < 2.0D)
        {
            double d0 = placer.posY + (double)placer.getEyeHeight();

            if (d0 - (double)pos.y > 2.0D)
            {
                return UP;
            }

            if ((double)pos.y - d0 > 0.0D)
            {
                return DOWN;
            }
        }
    ...
    {
        return byHorizontalIndex(MathHelper.floor_double((double)(entity.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3);
    }
